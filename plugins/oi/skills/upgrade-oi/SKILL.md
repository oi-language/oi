...
Bind typed caller inputs to the host-only lowercase resultless entry exactly as `func main(source fs.Path, targetVersion text, installed []text, available []text)`. Map reachable `fs.Scan` and `fs.Read` to `workspace.read`. Route `upgrade.CompileSource(source fs.Path, available []text) upgrade.CompileResult` to `compile-oi`, mapping named status values and the typed module, authorities, and diagnostics fields without reinterpretation. Map `effect Reply(value report.UpgradeReport) unit` to `caller.reply`.

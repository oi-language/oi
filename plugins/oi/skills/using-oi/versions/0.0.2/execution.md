...
`:=`/`var` declare mutable locals; const/parameters are not. Root must be a live mutable local; struct/slice selectors rebuild it. Map/set held by one or an assignable struct field permits whole-value rebind (`m = put(m,k,v)`). Map index, set element/member, text index, and handles forbid assignment/in-place mutation; collections update only through returned `append`/`put`/`add`/`remove` values. Assignment/arguments require identical type; literals need unique target; named conversion is explicit. `if` is bool; switch evaluates once, first equal, final default, no fallthrough. Loop entry snapshots: slice value or ascending index/value; map canonical key/value; set canonical value. Rebinding source does not change the encounter. `break` targets loop/switch/select; `continue` loop.

...
`channel[T](capacity)` requires durable `T` and compile-time positive integer capacity within policy. Otherwise `INVALID_CHANNEL_CAPACITY` uses the capacity expression. 0.0.1 has no zero-capacity/unbuffered channel.
...
0.0.1 has no `close`. Owner end journals channel end after attached join/cancellation; no endpoint survives. Stop/cancellation wakes blocked operations before terminal completion and propagates existing failure without inventing messages.

...
The target starts from its named function entry in a fresh context containing only target-reachable source and shards, the typed serialized arguments, intersected authorities, filtered mappings, and fresh trace, checkpoints, journal view, and channel/select counters. It excludes the old conversation, locals, attached-task results not passed as arguments, tool output, effect results, implicit memory, trace, checkpoints, journal, and unfiltered mappings.

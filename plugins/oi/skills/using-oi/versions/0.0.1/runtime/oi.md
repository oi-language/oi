...
Child context contains only target-reachable source, selected shards, explicit serialized arguments, narrowed authorities, filtered mappings, and fresh trace, checkpoints, and journal. It excludes parent conversation, locals, tool output, effect results, implicit memory, trace, checkpoints, journal, and unfiltered mappings. Values enter only by typed parameters or a specified runtime primitive.
